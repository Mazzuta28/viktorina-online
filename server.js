...
async function readBody(req) {
  ...
  for await (const chunk of req) { size += chunk.length; if (size > 1_000_000) throw new Error('too-large'); chunks.push(chunk); }
...
function roomState(room){
  return {code:room.code,quizTitle:room.quiz.title,status:room.status,totalQuestions:room.quiz.questions.length,participants:leaderboard(room)};
}
function rankFor(room,participantId){
  const board=leaderboard(room); const item=board.find(x=>x.id===participantId);
  return {rank:item?.rank||board.length,totalPlayers:board.length};
}
function pointsForAnswer(correct,responseMs){
  if(!correct) return 0;
  // 1000 очков за правильный ответ + до 500 очков за скорость.
  // После 30 секунд остаются только базовые 1000 очков.
...
function serveFile(res,file){
  ...
  fs.readFile(file,(err,data)=>{if(err)return text(res,404,'Не найдено');res.writeHead(200,{'content-type':MIME[path.extname(file)]||'application/octet-stream'});res.end(data);});
}
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(pathname==='/api/rooms' && req.method==='POST'){
      ...
      let quiz;
      try {
        // Для интернет-версии учитель отправляет викторину целиком при запуске.
        // Это позволяет хранить готовые викторины в браузере учителя и не зависеть
        // от временной файловой системы бесплатного хостинга.
      ...
      const c=code(),teacherToken=id('t_');rooms.set(c,{code:c,teacherToken,quiz,status:'open',createdAt:Date.now(),participants:new Map(),listeners:new Set()});return json(res,200,{code:c,teacherToken,quizTitle:quiz.title});
    }
    m=pathname.match(/^\/api\/rooms\/(\d{6})$/);
    if(m && req.method==='GET'){const room=rooms.get(m[1]);if(!room||room.status==='closed')return json(res,404,{error:'Комната не найдена или уже закрыта'});return json(res,200,{code:room.code,quizTitle:room.quiz.title,status:room.status,questionCount:room.quiz.questions.length});}
    ...
    if(m && req.method==='GET'){
      ...
      res.writeHead(200,{'content-type':'text/event-stream; charset=utf-8','cache-control':'no-cache','connection':'keep-alive','access-control-allow-origin':'*'});res.write(`data: ${JSON.stringify(roomState(room))}\n\n`);room.listeners.add(res);req.on('close',()=>room.listeners.delete(res));return;
    }
    m=pathname.match(/^\/api\/rooms\/(\d{6})\/join$/);
    if(m && req.method==='POST'){
      const room=rooms.get(m[1]);if(!room||room.status!=='open')return json(res,404,{error:'Комната не найдена или уже закрыта'});const body=await readBody(req);const name=String(body.name||'').trim().slice(0,40);if(!name)return json(res,400,{error:'Введите имя'});
      const now=Date.now();
      const p={id:id('p_'),name,joinedAt:now,order:shuffle(room.quiz.questions.map((_,i)=>i)),optionOrders:{},answers:[],current:0,finished:false,score:0,totalResponseMs:0,questionStartedAt:now};room.quiz.questions.forEach((q,i)=>{if(q.type==='choice')p.optionOrders[i]=shuffle(q.options.map((_,j)=>j));});room.participants.set(p.id,p);broadcast(room);return json(res,200,{participantId:p.id,quizTitle:room.quiz.title,questions:makeStudentQuestions(room,p),rank:rankFor(room,p.id).rank,totalPlayers:room.participants.size});
    }
    m=pathname.match(/^\/api\/rooms\/(\d{6})\/answer$/);
    if(m && req.method==='POST'){
      const room=rooms.get(m[1]);if(!room||room.status!=='open')return json(res,410,{error:'Викторина завершена'});const body=await readBody(req);const p=room.participants.get(String(body.participantId||''));if(!p||p.finished)return json(res,400,{error:'Участник не найден или уже закончил'});
      const qi=p.order[p.current],q=room.quiz.questions[qi];if(!q||q.id!==body.questionId)return json(res,409,{error:'Неверная последовательность вопроса'});let correct=false,answer=body.answer;
      if(q.type==='choice'){answer=Number(answer);correct=Number.isInteger(answer)&&answer===q.correctIndex;}else{answer=numberValue(String(answer??''));if(answer===null)return json(res,400,{error:'Введите целое число или конечную десятичную дробь',invalid:true});correct=Math.abs(answer-q.correctAnswer)<1e-12;}
    ...
    if(m && req.method==='GET'){
      ...
      if(participantId){
        ...
        const place=rankFor(room,p.id);
        return json(res,200,{status:'open',rank:place.rank,totalPlayers:place.totalPlayers,score:p.score||0,correct:p.answers.filter(a=>a.correct).length,answered:p.answers.length,total:room.quiz.questions.length,finished:p.finished});
      }
      return json(res,200,{status:'open'});
    }
    m=pathname.match(/^\/api\/rooms\/(\d{6})\/close$/);
    if(m && req.method==='POST'){
      const room=rooms.get(m[1]);if(!room)return json(res,404,{error:'Комната не найдена'});const body=await readBody(req);if(body.teacherToken!==room.teacherToken)return json(res,403,{error:'Нет доступа'});room.status='closed';broadcast(room);setTimeout(()=>{closeSse(room);rooms.delete(room.code);},3000);return json(res,200,{ok:true});
    }
